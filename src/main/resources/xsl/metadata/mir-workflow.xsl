...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                xmlns:fileCheck="xalan://de.vzg.leopoldina.LeopoldinaEditorUtil"
  ...
  <xsl:param name="MIR.Sherpa.API.Key" />
        ...
        <p>
          ...
          <ul>
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:if test="structure/derobjects/derobject/maindoc">
                  <xsl:variable name="fileNameCheck" select="fileCheck:getFileWarning($id)" />
                  <xsl:if test="string-length($fileNameCheck)&gt;0">
                    <li class="text-danger"><xsl:value-of select="$fileNameCheck" /></li>
                  </xsl:if>
                </xsl:if>
