        ...
        <ul class="dropdown-menu dropdown-menu-right">
          ...
          <xsl:choose>
            ...
            <xsl:otherwise>
              ...
              <!--
                TODO:
                check the both follwing conditions
                should be solved by a otherwise block
                or is it possible, that both are true?
                But:
                perhaps it is the best way to get a structure that is not nested
                when you have condition for each entry without otherwise blocks
              -->
              ...
              <xsl:if test="string-length($child-layout) &gt; 0 and $accessedit and mcrxsl:resourceAvailable('actionmappings.xml')">
                ...
                <xsl:choose>
                  <xsl:when test="not(contains($url, 'editor-dynamic.xed')) and $mods-type != 'series'">
                    <xsl:for-each select="str:tokenize($child-layout,'|')">
                      <li class="mir-list-child-actions">
                        <a href="{$url}{$HttpSession}?relatedItemId={$id}&amp;relatedItemType=host&amp;genre={.}" class="dropdown-item mir-list-indent mir-list-marked">
                  ...
                  <xsl:when test="not(contains($url, 'editor-dynamic.xed')) and $mods-type = 'series'">
                    <xsl:for-each select="str:tokenize($child-layout,'|')">
                      <li class="mir-list-child-actions">
                        <a href="{$url}{$HttpSession}?relatedItemId={$id}&amp;relatedItemType=series&amp;genre={.}" class="dropdown-item mir-list-indent mir-list-marked">
                          <xsl:value-of select="mcrxsl:getDisplayName('mir_genres',.)" />
                        </a>
                      </li>
                    </xsl:for-each>
                  ...
                  <xsl:when test="contains($url, 'editor-dynamic.xed') and $mods-type = 'lecture'">
                    <xsl:for-each select="str:tokenize($child-layout,'|')">
                      <li class="mir-list-child-actions">
                        <a href="{$url}{$HttpSession}?relatedItemId={$id}&amp;relatedItemType=series&amp;genre={.}&amp;host={$mods-type}" class="dropdown-item mir-list-indent mir-list-marked">
                          <xsl:value-of select="mcrxsl:getDisplayName('mir_genres',.)" />
                        </a>
                      </li>
                    </xsl:for-each>
